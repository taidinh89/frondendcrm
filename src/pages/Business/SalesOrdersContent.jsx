// src/pages/Business/SalesOrdersContent.jsx
...
// --- CẤU HÌNH CỘT MẶC ĐỊNH (SALES) ---
// Đây là nơi bạn có thể dễ dàng tùy chỉnh các cột hiển thị trong bảng dữ liệu.
//
// key:     Tên trường dữ liệu (field) từ API mà cột này sẽ hiển thị.
// label:   Nhãn (tên) của cột sẽ được hiển thị trên giao diện người dùng.
// width:   Chiều rộng mặc định của cột, tính bằng pixel.
// visible: `true` nếu cột được hiển thị mặc định, `false` nếu bị ẩn. Người dùng có thể thay đổi trong phần cài đặt.
// sortable: `true` nếu người dùng có thể sắp xếp bảng bằng cách nhấp vào tiêu đề cột này.
// format:  Định dạng hiển thị dữ liệu. Các tùy chọn: 'date', 'datetime', 'currency'.
// align:   Căn lề cho nội dung cột. Các tùy chọn: 'left', 'right', 'center'.
const DEFAULT_COLUMNS = [
    ...
    { key: 'ma_khncc', label: 'Mã KH', width: 120, visible: true, sortable: true },
    { key: 'ten_khncc', label: 'Tên Khách Hàng', width: 300, visible: true, sortable: true },
    { key: 'tong_cong', label: 'Tổng Tiền', width: 160, visible: true, sortable: false, align: 'right', format: 'currency' },
    { key: 'nguoi_phu_trach', label: 'NV Phụ Trách', width: 180, visible: true, sortable: true },
    { key: 'dia_chi_giao_hang', label: 'Địa Chỉ Giao', width: 300, visible: true, sortable: false },
    { key: 'hien_trang', label: 'Trạng Thái', width: 150, visible: true, sortable: true, align: 'center' },
    { key: 'ghi_chu_tren_phieu', label: 'Ghi Chú', width: 200, visible: false, sortable: false },
...
export const SalesOrdersContent = () => {
    // --- STATE ---
    ...
    // Load Config (Key V3)
    const STORAGE_KEY = 'sales_cols_v3';
    ...
    const { data: orders, isLoading, isFetchingMore, applyFilters, refresh, fetchNextPage } =
        useV2Paginator('/api/v2/sales-orders', { ...filters, sort_by: sortConfig.key, sort_direction: sortConfig.direction });
    ...
    // Column Logic (Resize, Move, Toggle)
        ...
        <div className="p-4 h-full flex flex-col bg-gray-100 overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-3 p-3 bg-white rounded shadow-sm shrink-0">
                ...
                <div className="flex items-center gap-2 flex-1 min-w-[300px]">
                    <Input placeholder="Tìm số phiếu, Khách hàng..." className="w-full" value={filters.search} onChange={e => setFilters({ ...filters, search: e.target.value })} onKeyDown={e => e.key === 'Enter' && applyFilters(filters)} />
                            ...
                            <div className="absolute right-0 top-full mt-2 w-72 bg-white border rounded shadow-xl z-50 p-2 max-h-[60vh] overflow-auto">
                                ...
                                {columns.map((c, i) => (
                                    <div key={c.key} className="flex justify-between p-1 hover:bg-gray-50 items-center">
                                        ...
                                        <div className="flex gap-1"><button onClick={() => moveColumn(i, -1)} disabled={i === 0} className="px-1 bg-gray-100 rounded text-xs">↑</button><button onClick={() => moveColumn(i, 1)} disabled={i === columns.length - 1} className="px-1 bg-gray-100 rounded text-xs">↓</button></div>
                                    </div>
                                ))}
                            </div>
                        )}
                                ...
                                <tr key={row.id} className="hover:bg-blue-50 group">
                                    ...
                                    <td className="p-2 border-r bg-white group-hover:bg-blue-50 sticky left-0 z-10 text-center shadow-sm">
                                        <div className="flex justify-center gap-1">
                                            <button
                                                onClick={() => setViewingDetailId(row.id)} // <--- LOGIC CŨ: Dùng ID
            ...
            {viewingDetailId && (
                <SalesOrderDetailModal
                    orderIdentifier={viewingDetailId} // Truyền ID số cho Modal
