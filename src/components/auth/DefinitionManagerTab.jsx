...
const DefinitionManagerTab = () => {
    ...
    const fetchDefinitions = async () => {
        try {
            const res = await axios.get('/api/v2/security/definitions');
    ...
    const handleSave = async () => {
        try {
            if (modal.data) {
                await axios.put(`/api/v2/security/definitions/${modal.data.id}`, formData);
                toast.success("Đã cập nhật định nghĩa");
            } else {
                await axios.post('/api/v2/security/definitions', formData);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`/api/v2/security/definitions/${id}`);
        ...
        <div className="animate-fadeIn space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {definitions.map(item => (
                    <div key={item.id} className={`p-5 rounded-2xl border-2 transition-all bg-white shadow-sm hover:shadow-md ${item.status === 'active' ? 'border-green-100' : 'border-gray-200'
                        }`}>
                        <div className="flex justify-between items-start mb-4">
                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase ${item.type === 'scope' ? 'bg-indigo-100 text-indigo-700' : 'bg-orange-100 text-orange-700'
                                }`}>{item.type}</span>

                            {/* ĐÈN TỰ ĐỘNG: XANH (ACTIVE) - XÁM (PENDING) */}
                            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border ${item.status === 'active'
