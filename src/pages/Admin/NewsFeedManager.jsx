...
// Helper to handle image URLs from local storage/relative paths
const getImageUrl = (url) => {
    if (!url) return '';
    if (url.startsWith('http')) return url;
    // Remove leading slash if exists
    const cleanUrl = url.startsWith('/') ? url.substring(1) : url;
    // Most posts are in storage/uploads/... but API might return uploads/...
...
export default function NewsFeedManager() {
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-10 animate-fade-in bg-slate-50/50 min-h-screen">
            <div className="flex justify-between items-end">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <span className="bg-blue-600 text-white text-[9px] font-black px-2 py-0.5 rounded italic">MODERATION</span>
                        <h1 className="text-5xl font-black text-slate-800 tracking-tighter italic uppercase">News Feed <span className="text-blue-600">Admin</span></h1>
                    </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex gap-10 px-8">
                        <div className="text-center">
                            <div className="text-2xl font-black text-slate-800 tracking-tighter">{posts.length}</div>
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng bài</div>
                        </div>
                        <div className="text-center border-l border-slate-100 pl-10">
                            <div className="text-2xl font-black text-emerald-500 tracking-tighter">{posts.filter(p => p.status === 'active' || p.status === 'published').length}</div>
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Đang hiện</div>
                        </div>
                    </div>
                    <a href="/news-feed-v4" target="_blank" className="bg-slate-800 text-white px-6 py-4 rounded-3xl font-black text-[11px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 flex items-center gap-2">
                        📱 Xem bản Mobile
                    </a>
                </div>
            </div>
            ...
            {/* Create Post Form */}
            <div className="bg-white rounded-[40px] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 flex flex-col gap-4">
                <textarea
                    value={content}
                ...
                {selectedImages.length > 0 && (
                    <div className="flex gap-4 overflow-x-auto py-2 px-2 scrollbar-hide">
                        {selectedImages.map((img, idx) => (
                            <div key={idx} className="relative w-28 h-28 rounded-2xl overflow-hidden border-4 border-white shadow-xl shrink-0 group">
                                <img src={img.url || img.preview_url} className="w-full h-full object-cover" alt="" />
                                <button
                                    onClick={() => setSelectedImages(prev => prev.filter((_, i) => i !== idx))}
                                    className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-100 hover:scale-110 transition-all shadow-lg text-[10px]"
                                >✕</button>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex justify-between items-center border-t border-slate-100 pt-4 mt-2">
                    ...
                    <button
                        ...
                        className="px-10 py-3 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-200 font-black text-[13px] uppercase tracking-wider hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
                {posts.map(post => {
                    const isVisible = post.status === 'active' || post.status === 'published';
                    return (
                        <div key={post.id} className={`bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden flex flex-col transition-all hover:shadow-2xl hover:border-blue-100 group ${!isVisible ? 'opacity-60 bg-slate-50' : ''}`}>
                            {/* Post Header */}
                            <div className="p-6 pb-4 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-100 overflow-hidden border-2 border-white shadow-md">
                                        {post.user?.avatar ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-black">
                                ...
                                <div className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${isVisible ? 'bg-emerald-50 text-emerald-500' : 'bg-slate-200 text-slate-500'}`}>
                            ...
                            <div className="px-6 flex-1">
                                ...
                                {post.images && post.images.length > 0 && (
                                    <div className="grid grid-cols-2 gap-2 mb-4 rounded-3xl overflow-hidden relative group-hover:grayscale-0 transition-all duration-500">
                                        ...
                                        {post.images.length > 4 && (
                                            <div className="absolute inset-0 flex items-center justify-center bg-black/40 pointer-events-none">
                                                <span className="text-white text-2xl font-black">+{post.images.length - 4}</span>
                                            </div>
                                        )}
                                    </div>
                                )}
                            ...
                            <div className="p-6 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg transition-all ${isVisible ? 'bg-white text-slate-400 hover:text-amber-500 hover:rotate-12' : 'bg-emerald-500 text-white shadow-emerald-200 animate-pulse'}`}
                                    ...
                                    <button
                                        ...
                                        className="w-10 h-10 bg-white text-red-500 rounded-2xl shadow-lg flex items-center justify-center hover:bg-red-500 hover:text-white transition-all disabled:opacity-50"
                                    >
                ...
                {posts.length === 0 && (
                    <div className="col-span-full py-40 text-center bg-white border-2 border-dashed border-slate-100 rounded-[60px]">
                        ...
                        <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-[10px]">Bảng tin hiện đang trống</p>
                    </div>
                )}
            </div>
        </div>
